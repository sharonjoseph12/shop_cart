...
const CartierRotor = ({ position, speed = 1 }) => {
  const ref = useRef();
  useFrame((state) => {
    ref.current.rotation.y = state.clock.elapsedTime * 6 * speed;
  });
  return (
...
export const Drone3D = ({ scrollRef = { current: 0 } }) => {
  const group = useRef();
  ...
  useFrame((state) => {
    ...
    if (sp < 0.05) {
      ...
      group.current.position.y = hoverY + sp * 3.5;
      group.current.rotation.z = hoverRotZ + sp * 0.4;
    }

    group.current.position.x = sp * 2;
    group.current.rotation.y = Math.sin(t * 0.15) * 0.1 + sp * Math.PI;
  ...
  return (
    <group ref={group} scale={[0.7, 0.7, 0.7]}>
      ...
      <mesh>
        ...
        <meshPhysicalMaterial color="#0A0A0A" metalness={0.95} roughness={0.08} clearcoat={1} clearcoatRoughness={0.1} envMapIntensity={2} />
      </mesh>
      ...
      <mesh position={[0, 0.05, 0]}>
        ...
        <meshPhysicalMaterial color="#C5A455" metalness={0.9} roughness={0.12} envMapIntensity={2.5} />
      </mesh>
      <mesh position={[0.3, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
        <coneGeometry args={[0.09, 0.22, 12]} />
        <meshPhysicalMaterial color="#1A1A1A" metalness={0.85} roughness={0.15} envMapIntensity={1.5} />
      </mesh>
      ...
      {[[0.14, 0.045, 0.14], [-0.14, 0.045, 0.14], [0.14, -0.045, 0.14], [-0.14, -0.045, 0.14]].map((pos, i) => (
      ...
      <group ref={capsuleRef} position={[0, -0.18, 0]}>
        <mesh>
          ...
          <meshPhysicalMaterial color="#F5F0EB" transparent opacity={0.08} roughness={0} metalness={0} clearcoat={1} transmission={0.9} ior={1.5} thickness={0.4} envMapIntensity={2} />
        </mesh>
