...
const CustomerDashboard = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <Reveal variant="fadeUp">
        <div className="mb-10 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          <div>
            ...
            <p className="text-[#F5F0EB]/30 text-sm font-sans-luxury mt-1">Welcome back, <span className="text-[#C8102E]">{user?.email}</span></p>
          </div>
      ...
      <motion.div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-10" variants={stagger} initial="hidden" animate="show">
        <motion.div variants={fadeUp} className="glass-card p-6 flex items-center gap-4 group hover:border-[#C8102E]/20 transition-all">
          <div className="p-3 bg-[#C8102E]/10 text-[#C8102E] rounded-xl"><FiShoppingBag size={20} /></div>
          <div>
        ...
        <motion.div variants={fadeUp} className="glass-card p-6 flex items-center gap-4 group hover:border-[#C5A455]/30 transition-all">
          <div className="p-3 bg-[#C5A455]/10 text-[#C5A455] rounded-xl"><FiCheckCircle size={20} /></div>
          <div>
        ...
        <motion.div variants={fadeUp} className="glass-card p-6 flex items-center gap-4 group hover:border-[#C8102E]/20 transition-all">
          <div className="p-3 bg-white/[0.04] text-[#F5F0EB]/40 rounded-xl"><FiPackage size={20} /></div>
          <div>
          ...
          {myOrders.length === 0 ? (
            ...
            <motion.div className="space-y-4" variants={stagger} initial="hidden" animate="show">
              {myOrders.map((order) => {
                ...
                return (
                  <motion.div key={order.id} variants={fadeUp} className="border border-white/[0.04] rounded-2xl p-6 hover:bg-white/[0.02] transition-all bg-white/[0.01]">
